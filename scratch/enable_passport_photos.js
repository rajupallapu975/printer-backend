...
async function toggleService() {
  ...
  for (const id of matchedIds) {
    ...
    await docRefCustomer.update({ isAvailable: true });
    await docRefAdmin.update({ isAvailable: true });
    console.log(`Enabled service: "${id}"`);
  }

  console.log(`\nSuccessfully enabled all ${matchedIds.length} passport size photo service documents in both databases!`);
